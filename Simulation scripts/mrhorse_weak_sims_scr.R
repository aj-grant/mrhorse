...
sim_dat = function(p, iv, n, theta, balanced_pl = T, nx){
  n1 = n + nx
  ...
  d[valid_gv] = 0
  maf = runif(p, 0.1, 0.3)
  G = sapply(1:p, function(j){rbinom(n1, 2, maf[j])})
  U = rnorm(n1, 0, 1) + G %*% d
  X = G %*% bx + U + rnorm(n1, 0, 2)
  Y = theta * X + G %*% a + U + rnorm(n1, 0, 1)
...
D_20 = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.2, n, theta = 0.1, nx = nx)
  return(A)
})

D_20_null = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.2, n, theta = 0, nx = nx)
  return(A)
})

D_20_dir = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.2, n, theta = 0.1, balanced_pl = F, nx = nx)
  return(A)
})

D_20_null_dir = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.2, n, theta = 0, balanced_pl = F, nx = nx)
  return(A)
})

clusterSetRNGStream(cl, 20220305)
D_40 = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.4, n, theta = 0.1, nx = nx)
  return(A)
})

D_40_null = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.4, n, theta = 0, nx = nx)
  return(A)
})

D_40_dir = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.4, n, theta = 0.1, balanced_pl = F, nx = nx)
  return(A)
})

D_40_null_dir = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.4, n, theta = 0, balanced_pl = F, nx = nx)
  return(A)
})

clusterSetRNGStream(cl, 20220305)
D_60 = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.6, n, theta = 0.1, nx = nx)
  return(A)
})

D_60_null = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.6, n, theta = 0, nx = nx)
  return(A)
})

D_60_dir = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.6, n, theta = 0.1, balanced_pl = F, nx = nx)
  return(A)
})

D_60_null_dir = parLapply(cl, 1:M, function(i){
  A = sim_dat(p, iv = 0.6, n, theta = 0, balanced_pl = F, nx = nx)
  return(A)
})
...
R_20 = parSapply(cl, 1:M, function(i){
  D = D_20[[i]]
  sim_res(D, p, nx, no_ini, variable.names)
})
